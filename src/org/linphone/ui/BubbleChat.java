		...
		LinphoneChatMessage.LinphoneChatMessageListener
{
			...
			LinphoneContact c)
	{
		...
		if (message.isOutgoing())
		{
			view = LayoutInflater.from(context).inflate(
					R.layout.chat_bubble_outgoing, null);
		} else
		{
			view = LayoutInflater.from(context).inflate(
					R.layout.chat_bubble_incoming, null);
		}
		view.setId(message.getStorageId());

		defaultBitmap = BitmapFactory.decodeResource(mContext.getResources(),
				R.drawable.chat_picture_over);
		...
		if (statusView != null)
		{
			...
			} else if (status == LinphoneChatMessage.State.NotDelivered)
			{
				...
				statusView
						.setImageResource(R.drawable.chat_message_not_delivered);
			} else
			{
		...
		LinphoneContent fileTransferContent = message
				...
				public void onClick(View v)
				{
					if (LinphoneManager.getInstance().getMessageUploadPending() != null)
					{
						...
						LinphoneManager.getInstance()
								.setUploadPendingFileMessage(null);
					}
				}
			});
		}
		...
		if (externalBodyUrl != null || fileTransferContent != null)
		{
			...
			if (nativeMessage.isOutgoing() && appData != null)
			{
				...
				RelativeLayout imageLayout = (RelativeLayout) view
						.findViewById(R.id.file_transfer_layout);
				if (LinphoneManager.getInstance().getMessageUploadPending() != null
						&& LinphoneManager.getInstance()
								.getMessageUploadPending().getStorageId() == nativeMessage
								.getStorageId())
				{
			...
			{
				if (appData != null
						&& !LinphoneManager.getInstance().isMessagePending(
								nativeMessage)
						&& appData
								.contains(context.getString(
										R.string.temp_photo_name_with_date)
										.split("%s")[0]))
				{
					appData = null;
				}
				...
				RelativeLayout imageLayout = (RelativeLayout) view
						.findViewById(R.id.file_transfer_layout);
				acceptDownload = (Button) view
						.findViewById(R.id.accept_download);
					...
					acceptDownload.setOnClickListener(new OnClickListener()
					{
						...
						public void onClick(View v)
						{
									...
									mContext.getPackageName()) == PackageManager.PERMISSION_GRANTED)
							{
								...
								LinphoneManager.getInstance()
										.addDownloadMessagePending(
												nativeMessage);
								nativeMessage.setListener(LinphoneManager
										.getInstance());
								nativeMessage.setFileTransferFilepath(file
										.getPath());
				...
				{
					if (LinphoneManager.getInstance().isMessagePending(
							nativeMessage))
					{
						LinphoneManager.addListener(this);
		...
		if (!nativeMessage.isOutgoing())
		{
			if (c != null)
			{
				...
				LinphoneUtils.setImagePictureFromUri(view.getContext(),
						contactPicture, c.getPhotoUri(), c.getThumbnailUri());
			} else
			{
		...
		contact.setText(timestampToHumanDate(context, message.getTime())
				+ " - " + displayName);
	...
	private String timestampToHumanDate(Context context, long timestamp)
	{
		try
		{
			...
			if (isToday(cal))
			{
				dateFormat = new SimpleDateFormat(context.getResources()
						.getString(R.string.today_date_format));
			} else
			{
				dateFormat = new SimpleDateFormat(context.getResources()
						.getString(R.string.messages_date_format));
			}
		...
		for (Entry<String, Integer> entry : emoticons.entrySet())
		{
			...
			while (indexOf >= 0)
			{
				...
				builder.setSpan(new ImageSpan(context, entry.getValue()),
						indexOf, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	public static Spanned getTextWithHttpLinks(String text)
	{
		...
		if (text.contains("http://"))
		{
			...
			int indexFinHttp = text.indexOf(" ", indexHttp) == -1 ? text
					.length() : text.indexOf(" ", indexHttp);
			String link = text.substring(indexHttp, indexFinHttp);
			String linkWithoutScheme = link.replace("http://", "");
			text = text.replaceFirst(link, "<a href=\"" + link + "\">"
					+ linkWithoutScheme + "</a>");
		}
					...
					imageView.setOnClickListener(new OnClickListener()
					{
						...
						public void onClick(View v)
						{
							...
							intent.setDataAndType(
									Uri.parse((String) v.getTag()), "image/*");
