 ...
 */
public class RemoteProvisioningLoginActivity extends Activity implements
		OnClickListener
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		mListener = new LinphoneCoreListenerBase()
		{
			...
			public void configuringStatus(LinphoneCore lc,
					final LinphoneCore.RemoteProvisioningState state,
					String message)
			{
				...
				{
					Toast.makeText(RemoteProvisioningLoginActivity.this,
	...
	private void cancelWizard(boolean bypassCheck)
	{
		if (bypassCheck
				|| getResources().getBoolean(
						R.bool.allow_cancel_remote_provisioning_login_activity))
		{
			LinphonePreferences.instance().disableProvisioningLoginView();
			setResult(bypassCheck ? Activity.RESULT_OK
	...
	private boolean storeAccount(String username, String password, String domain)
	{
				...
				new XmlRpcListenerBase()
				{
					...
					public void onRemoteProvisioningFilenameSent(String result)
					{
						LinphonePreferences.instance()
								.setRemoteProvisioningUrl(result);
		 ...
		 * LinphoneProxyConfig prxCfg = lc.createProxyConfig(); try {
		 * prxCfg.setIdentity(identity); lc.addProxyConfig(prxCfg); } catch
		 * (LinphoneCoreException e) { Log.e(e); return false; }
		 * 
		 * LinphoneAuthInfo authInfo =
		 * LinphoneCoreFactory.instance().createAuthInfo(username, null,
		 * password, null, null, domain); lc.addAuthInfo(authInfo);
		 * 
	...
	public void onClick(View v)
	{
		...
		{
			storeAccount(login.getText().toString(), password.getText()
					.toString(), domain.getText().toString());
		}
	}
