		...
		ActivityCompat.OnRequestPermissionsResultCallback
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		firstFragment = getResources().getBoolean(
				R.bool.assistant_use_linphone_login_as_first_fragment) ? AssistantFragmentsEnum.LINPHONE_LOGIN
		...
		mListener = new LinphoneCoreListenerBase()
		{
					...
					LinphoneCore.RegistrationState state, String smessage)
			{
				...
				{
					if (address != null
							&& address.asString().equals(
									cfg.getAddress().asString()))
					{
	...
	private void logIn(String username, String password, String displayName,
	...
	public void checkAccount(String username, String password,
			String displayName, String domain)
	{
		saveCreatedAccount(username, password, displayName, domain, null);
	}

	public void linphoneLogIn(String username, String password,
			String displayName, boolean validate)
	{
		if (validate)
		{
			checkAccount(username, password, displayName,
					getString(R.string.default_domain));
		} else
		{
			if (accountCreated)
			{
				retryLogin(username, password, displayName,
						getString(R.string.default_domain), null);
			} else
			{
				logIn(username, password, displayName,
						getString(R.string.default_domain), null, true);
			}
		}
	}

	public void genericLogIn(String username, String password,
			String displayName, String domain, TransportType transport)
	{
		if (accountCreated)
		{
	...
	public void retryLogin(String username, String password,
			String displayName, String domain, TransportType transport)
	{
			...
			String displayName, String domain, TransportType transport)
	{
		...
		try
		{
			address = LinphoneCoreFactory.instance().createLinphoneAddress(
					identity);
		...
		boolean isMainAccountLinphoneDotOrg = domain
				.equals(getString(R.string.default_domain));
		AccountBuilder builder = new AccountBuilder(LinphoneManager.getLc())
				.setUsername(username).setDomain(domain)
				.setDisplayName(displayName).setPassword(password);
		...
		if (isMainAccountLinphoneDotOrg)
		{
			...
			builder.setExpires("604800")
					...
					.setRealm("sip.linphone.org").setNoDefault(false);
	...
	public void displayRegistrationInProgressDialog()
	{
		if (LinphoneManager.getLc().isNetworkReachable())
		{
			...
			Drawable d = new ColorDrawable(getResources().getColor(
					R.color.colorE));
			d.setAlpha(200);
	...
	{
		Toast.makeText(this, getString(R.string.assistant_account_validated),
		...
		builder.setMessage(message)
				...
				.setNegativeButton(getString(R.string.cancel),
						new DialogInterface.OnClickListener()
						{
							public void onClick(DialogInterface dialog, int id)
							{
								LinphoneManager.getLc().removeProxyConfig(
